...
namespace VEngraveForCamBam
{
    ...
    public class MOPVEngrave : MOPFromGeometry, IIcon
    {
        ...
        // added [NonSerialized] for CamBAm v1.0 Aug2018
        [NonSerialized]
        ...
        #region UIProperties
        // UI properties
        // set up icons for the user interface
        [XmlIgnore]
        ...
        #region DepthIncrementProperty
        // July 2021 Eddy added new parameter "Depth Increment"
        [DefaultValue(typeof(CBValue<double>), "Default")]
        [Category("Cutting Depth")]
        [Description("Depth increment of each machining pass.")]
        [DisplayName("Depth Increment")]
        [CBKeyValue]
        ...
        public CBValue<double> DepthIncrement
        {
            get
            {
                if (_depthIncrement.IsAuto)
                {
                    _depthIncrement.SetCache(0);
                }
            ...
            set
            {
                ...
                {
                    _depthIncrement.SetState(CBValueStates.Error);
                    _log.Log(ERROR, "DepthIncrement must be greater than zero.");
                    return;
                }
                _depthIncrement = value;
            }
        }

        public bool ShouldSerializeDepthIncrement()
        {
            return DepthIncrement.ShouldSerialize;
        }

        [XmlElement("DepthIncrement")]
        private CBValue<double> _depthIncrement;
        private CBValue<double> mds; 

        #endregion DepthIncrementProperty
        ...
        [DisplayName("Path Step Size"), Description("Length of step in path"), Category("Options"), CBAdvancedValue]
        public CBValue<double> PathIncrement
        {
            set
            {
                ...
                {
                    Units drawingUnits = _CADFile != null ? _CADFile.DrawingUnits : Units.Unknown;
                    switch (drawingUnits)
                    {
        ...
        {
            _log.Log(FINEST, "ShouldSerializePathIncrement(): {0}", !_pathIncrement.IsDefault);
        ...
        {
            _log.Log(FINEST, "ShouldSerializeMaxCornerAngle(): {0}", !_maxCornerAngle.IsDefault);
        ...
        [DisplayName("Tool Tip Diameter"), Description("Tip diameter of engraving tool"), DefaultValue(DEFAULT_TOOL_TIP_DIAMETER), Category("Tool")]
        ...
        {
            _log.Log(FINEST, "ShouldSerializeToolTipDiameter(): {0}", !_toolTipDiameter.IsDefault);
        ...
        [DisplayName("Tool V-Angle"), Description("V-angle of engraving tool"), DefaultValue(DEFAULT_TOOL_V_ANGLE), Category("Tool")]
        ...
        {
            _log.Log(FINEST, "ShouldSerializeToolVAngle(): {0}", !_toolVAngle.IsDefault);
        ...
        {
            // EddyCurrent - if the ToolNumber changes then also update the VAngle parameter
            if (propertyname == "ToolNumber")
            {
                if (CurrentTool.VeeAngle > 0 && CurrentTool.VeeAngle <= 180)
                {
                    ToolVAngle = new CBValue<double>(CurrentTool.VeeAngle);
                }
                else _toolVAngle.SetState(CBValueStates.Default);
            }
            // EddyCurrent
        ...
        public MOPVEngrave()
        {
            ...
            // July 2021
            DepthIncrement = CBValue<double>.Default; 
        }
        ...
        public MOPVEngrave(CADFile file, object[] objects) : base(file, objects) { }
        ...
        //  main part that is called when create toolpaths is requested
        ...
        // Eddy July 2021 modified the method "CreateToolpathSequence" to provide stepped depth increment toolpaths
        //
        ...
        {
            ToolpathSequence tseq = new ToolpathSequence(this);
            ToolpathSequence disposable = new ToolpathSequence(this);

            // keep a copy of initial value of MaxDepth because it will change during the course of this code
            double md = MaxDepth.Cached;

            // remember the initial state of _maxDepth, we shall have to reset it back at the end because we changed it to a "value" in between
            if (_maxDepth.IsValue) mds.SetState(CBValueStates.Value);
            if (_maxDepth.IsDefault) mds.SetState(CBValueStates.Default);
            if (_maxDepth.IsAuto) mds.SetState(CBValueStates.Auto);
            
            // get value of Depth Increment
            double depthInc = DepthIncrement.Cached;
            
            foreach (ShapeListItem item in shapes)
            {
                // do a dummy run to get deepest point in the item, required for loop below
                _maxDepth = new CBValue<double>(md);
                // reset lowestpoint
                Params.lowestPoint = 0;
                // use disposable toolpathsequence that will not be used for actual toolpaths
                // Params.lowestPoint will be set in, ComputeVEngraveToolpath->FollowOutline->AnalyzePoint 
                ComputeVEngraveToolpath(disposable, item);
                // Params.lowestPoint will now have value of the deepest point for this item

                // now continue with Toolpath sequence (tseq) to be used for actual toolpaths
                depthInc = DepthIncrement.Cached;

                // if depthInc = 0 we need to regard that as "go immediately to full depth cut"
                if (depthInc > 0)
                {
                    // work through the depth levels
                    while (depthInc < Params.lowestPoint)
                    {
                        _maxDepth = new CBValue<double>(depthInc);
                        ComputeVEngraveToolpath(tseq, item);
                        // go to next depthincrement step
                        depthInc += DepthIncrement.Cached;
                    }
                }
                // need to catch any remainder and cut at full depth       
                // or if depthInc = 0
                if (depthInc >= Params.lowestPoint || depthInc == 0)
                {
                    _maxDepth = new CBValue<double>(md);
                    ComputeVEngraveToolpath(tseq, item);
                }
            ...
            //reset _maxDepth, however do not clear the cache of _maxDepth if "Value" state was selected
            if (mds.IsValue) _maxDepth.SetState(CBValueStates.Value);
            if (mds.IsDefault)
            {
                _maxDepth.ClearCache();
                _maxDepth.SetState(CBValueStates.Default);
            }
            if (mds.IsAuto)
            {
                CamBam.ThisApplication.AddLogMessage("Auto = " + _maxDepth);
                _maxDepth.ClearCache();
        ...
        internal List<Surface> ComputeCutSurfaces(List<ToolpathItem> tpil)
        {
            ...
            foreach (ToolpathItem tpi in tpil)
            {
                ...
                // added by pstemari (EddyCurrent)
                // This change stopped 'Show Cut Widths' working
               ...
               toolpath.Points[toolpath.NextSegment(0)].Point.To2D()))
                ...
                if (!toolpath.Closed)
                {
                    ComputeCutEndcap(current, Geometry.Multiply(-1, currstep), bob);
                }
                cutsurfaces.Add(bob.Build());
                // closing brace from line 545               }
            }
            return cutsurfaces;
        }
        ...
        internal void ComputeCutEndcap(Point3F endpoint, Vector2F openDirection, SurfaceBuilder bob)
        {
            ...
            Point3F prevTop = new Point3F(endpoint.X + topRadius * Math.Cos(startAngle), endpoint.Y + topRadius * Math.Sin(startAngle), StockSurface.Cached);
            Point3F prevBottom = new Point3F(endpoint.X + bottomRadius * Math.Cos(startAngle), endpoint.Y + bottomRadius * Math.Sin(startAngle), endpoint.Z);

            for (int i = 1; i <= nPoints; ++i)
            {
                double angle = startAngle + i * (endAngle - startAngle) / nPoints;
                Point3F currTop = new Point3F(endpoint.X + topRadius * Math.Cos(angle), endpoint.Y + topRadius * Math.Sin(angle), StockSurface.Cached);
                Point3F currBottom = new Point3F(endpoint.X + bottomRadius * Math.Cos(angle), endpoint.Y + bottomRadius * Math.Sin(angle), endpoint.Z);
        ...
        internal void ComputeVEngraveToolpath(ToolpathSequence tseq, ShapeListItem item)
        {
            ...
            if (item.Shape is Polyline)
            {
                ...
                FollowOutline(tseq, (Polyline)item.Shape, outlines, item.EntityID, parentID: -1, offsetIndex: ref offsetIndex, depthIndex: depthIndex, traceInside: true);
            }
            else if (item.Shape is Region)
            {
                ComputeRegionToolpaths(tseq, (Region)item.Shape, item.EntityID, ref depthIndex, ref offsetIndex);
            }
            else if (item.Shape is MText)
            {
                foreach (Region region in ((MText)item.Shape).ToRegions())
                {
                    ...
                    ComputeRegionToolpaths(tseq, region, item.EntityID, ref depthIndex, ref offsetIndex);
                }
            }
            else
            {
                _log.Log(ERROR, "Don't know what to do with {0} entity {1}", item.Shape.GetType(), item.EntityID);
            }
        }
        ...
        internal void FollowOutline(ToolpathSequence tseq, Polyline outline, List<Polyline> outlines, EntityIdentifier outlineID, int parentID, ref int offsetIndex, int depthIndex, bool traceInside)
        {
            double dl = PathIncrement.Cached;
            _log.Log(DEBUG, "Following polyline {0}, direction: {1}", outline.ID, outline.Direction);

            if (!outline.Closed)
            {
                _log.Log(ERROR, "I don't know what to do with open polyline {0}", outlineID);
            }
            else if (outline.Direction == RotationDirection.Unknown)
            {
                _log.Log(ERROR, "I don't know what to do with unknown rotation" + " polyline {0}", outlineID);
            }
            else
            {
                bool leftIsInside = (outline.Direction == RotationDirection.CCW) == traceInside;
                double thresholdAngle = MaxCornerAngle.Cached * DEGREES;
                PolylineItemList point = outline.Points;
                Polyline toolpath = new Polyline();
                Vector2F nextNormal;
                Geometry.CornerType startCornerType = Geometry.GetCornerType(outline, 0, thresholdAngle, leftIsInside, out nextNormal);
                ...
                {
                    _log.Log(TRACE, "Processing segment[{0}], starting point: {1}", i, point[i]);
                    ...
                    Geometry.CornerType endCornerType = Geometry.GetCornerType(outline, nexti, thresholdAngle, leftIsInside, out nextNormal);
                    ...
                    if (Math.Abs(bulge) < DBL_EPSILON)
                    {
                        ...
                        while (stepsLeft-- >= 0)
                        { // include both first and last points
                            double computedRadius = AnalyzePoint(outline, i, current, curNormal, Double.MaxValue, outlines, startCornerType, endCornerType, toolpath, leftIsInside);
                    ...
                    {
                        Point2F center;
                        double arcRadius = Geometry.ConvertBulgeToArc(start, end, bulge, out center);
                        int direction = Math.Sign(bulge);
                        double startTheta = Math.Atan2(start.Y - center.Y, start.X - center.X);
                        double endTheta = Geometry.NormalizeAngularInterval(direction, startTheta, Math.Atan2(end.Y - center.Y, end.X - center.X));
                        int stepsLeft = (int)Math.Max(Math.Abs((endTheta - startTheta) / (dl / arcRadius)), 1);
                ...
                if (Point3F.Distance(toolpath.Points[0].Point, toolpath.Points[toolpath.Points.Count - 1].Point) <= PathIncrement.Cached)
                {
                ...
                toolpath.InsertSegmentBefore(0, new PolylineItem(toolpath.FirstPoint.To2D().To3D(zEndpoints)));
                toolpath.Add(toolpath.LastPoint.To2D().To3D(zEndpoints));

                // Looks like these lines are making the tool do two passes when cutting - EddyCurrent
                //  I think they are here in error because they are duplicated further down.

                             //  _log.Log(DEBUG, "Adding {0} item toolpath for entity {1}", toolpath.Points.Count, outlineID);
                             //  tseq.Add(/* depthIndex */ 0, offsetIndex, outlineID, parentID, toolpath, outline.Direction, sourcepoint: Point3F.Undefined, zoffset: 0);
                ...
                if (toolpath.Points.Count > 0)
                {
                    ...
                    while (toolpath.Points.Count >= 2 && toolpath.FirstPoint.Z > StockSurface.Cached)
                    {
                        ...
                        {
                            double fraction = (StockSurface.Cached - firstPoint.Z) / (secondPoint.Z - firstPoint.Z);
                            ...
                            _log.Log(DEBUG, "Replacing point at {0} with {1}", firstPoint, newFirstPoint);
                            toolpath.Points.RemoveAt(0);
                            toolpath.InsertSegmentBefore(0, new PolylineItem(newFirstPoint));
                        }
                    }

                    while (toolpath.Points.Count >= 2 && toolpath.LastPoint.Z > StockSurface.Cached)
                    {
                        var firstPoint = toolpath.Points[toolpath.PrevSegment(toolpath.Points.Count - 1)].Point;
                        ...
                        else
                        {
                            ...
                            _log.Log(DEBUG, "Replacing point at {0} with {1}", secondPoint, newSecondPoint);
                            toolpath.Points.RemoveAt(toolpath.Points.Count - 1);
                            toolpath.Add(newSecondPoint);
                        }
                    }
                    // Close path if it loops
                    if (toolpath.Points.Count > 1 && Point3F.Distance(toolpath.Points[0].Point, toolpath.Points[toolpath.Points.Count - 1].Point) <= PathIncrement.Cached)
                    {
                        toolpath.Add(toolpath.Points[0].Point);
                    }
                    ...
                    {
                        toolpath.InsertSegmentBefore(0, new PolylineItem(toolpath.FirstPoint.To2D().To3D(StockSurface.Cached)));
                    }
                    ...
                    _log.Log(DEBUG, "Adding {0} item toolpath for entity {1}", toolpath.Points.Count, outlineID);
                    tseq.Add(0, offsetIndex++, outlineID, parentID, toolpath, outline.Direction, sourcepoint: Point3F.Undefined, zoffset: 0);
                }
            }
        }
            ...
            Polyline toolpath, bool leftIsInside)
        {
            ...
            Point3F toolpathPoint = new Point3F(current.X + radius * normal.X, current.Y + radius * normal.Y, z);
            if (toolpath.Points.Count >= 1 && Point3F.Distance(toolpath.LastPoint, toolpathPoint) < DBL_EPSILON)
            {
            ...
            // get lowest point in toolpath
            // *-1 because the z values are negative but we need positive
            if (z*-1 > Params.lowestPoint) Params.lowestPoint = z*-1;
        ...
        private double ComputeZFromRadius(ref double radius)
        {
            ...
            {
                _log.Log(TRACE, "Adjusting depth, radius = {0}, computed depth = {1}, " + "MaxDepth = {2}", radius, depth, MaxDepth.Cached);
